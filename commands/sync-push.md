...
1. If you changed config this session, run `bash ~/.claude/sync.sh save` first.
...
- settings.json is auto-resolved by the merge-settings.py driver: per-machine prefs kept local, all else merged.
  - A conflict THERE means a driver gap - a per-machine pref missing from PER_MACHINE.
  - Fix the set, not the file.
