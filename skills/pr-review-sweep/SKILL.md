...
4. Fan out in parallel: one subagent per lens, each in its own worktree, each told "no edits, findings only", each given the same PR context and a fixed return format.
   - Worktree facts (2026-08-07): they start on the default branch - first step in every prompt is
     `git switch --detach <head-sha>`; they isolate git state only, so machine-state checks can go to reviewers.
...
5. Vet every finding against the diff: confirm it cites a real `file:line` and reproduces on the production
   engine or build (a lookalike non-repro refutes nothing); drop the hallucinated or out-of-scope ones.
...
- One-line verdict.
- Findings, worst first: `[sev/conf] file:line - issue -> fix (in-scope: y/n)`.
