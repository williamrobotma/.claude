...
# Compact token counts: k is the floor unit, both values always carry their own
# suffix (0 -> 0k), promoting to M at 999500 (where %.0f would round to 1000k).
fmt_token_pair() {
  awk -v a="$1" -v b="$2" '
    function unit(v) { return (v >= 999500) ? sprintf("%.0fM", v / 1000000) : sprintf("%.0fk", v / 1000) }
    BEGIN { printf "%s/%s", unit(a), unit(b) }
  '
}
ctx_disp=""
[ -n "$used_tokens" ] && [ -n "$ctx_max" ] && [ "$ctx_max" != "0" ] && \
