...
- One-line docstring only when name + signature already say everything; never as a default.
...
- Docstrings beyond one line (Writing covers when one line suffices): Google-style Args/Returns/Raises, each field a phrase or short clause, not a prose paragraph.
...
- Docstrings and comments: minimize explanatory prose; if it can be said in a phrase, don't write a sentence.
- Keep comment/docstring text lines to <=80 chars. Only a short tag may sit inline (`# obsm, not the clipped .X`); anything longer goes on its own line above the code it describes. Don't put a prose cell above every step.
